...
- Clipboard tools live in [`./tools/clipboard-images/`](./tools/clipboard-images/): in-tree autosave listener (PR 8, source-only) and on-demand WezTerm Alt+V capture [`Capture-CurrentClipboardImage.ps1`](./tools/clipboard-images/Capture-CurrentClipboardImage.ps1) (`powershell.exe -NoProfile -STA -File`, optional `-DestinationDirectory`, unique PNGs under `%LOCALAPPDATA%\PodlePaste\staging`). Capture must not scan Screenshots2, mutate the clipboard, or start/stop the live PodleShell listener.
